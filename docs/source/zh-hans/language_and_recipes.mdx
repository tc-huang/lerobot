# 语言列与配方[[language-columns-and-recipes]]
...
## 第 1 层 — 数据集中的语言列[[layer-1-language-columns-in-the-dataset]]
...
## 第 2 层 — 配方结构[[layer-2-recipe-anatomy]]

配方是由来自 `lerobot.datasets.recipe` 的 `TrainingRecipe` 和 `MessageTurn` 支撑的 YAML 文件。它们
...
### 视角相关解析[[view-dependent-resolution]]
...
为数据集记录的每个相机添加一个这样的子配方。显式
`route: vqa` 标记会使匹配的稀疏 VQA 标注优先
于常规加权 blend 选择；组件名称纯属描述性。

## 第 3 层 — 训练格式[[layer-3-training-format]]
...
混合配方会从样本索引确定性地选择一个加权子配方。
`recipes/subtask_mem.yaml` 训练紧凑的核心混合 — 高层子任务预测、低层执行和记忆。`recipes/subtask_mem_vqa_speech.yaml` 是更完整的变体，还加入了 VQA 和语音插话响应。

`recipes/subtask_joint.yaml` 展示了联合序列训练，而不是加权
混合。对于同一个样本，其 assistant 子任务会在 `low_level` 流上以
文本交叉熵进行监督，同时动作预测保持
激活，匹配 π0.5 论文中的联合设置。启用
`--policy.joint_subtask_conditioning=true`，以便在推理时使用该子任务条件。

## 优雅缺省[[graceful-absence]]

如果两个语言列都缺失、为 `None` 或为空，`RenderMessagesStep` 会在可用时将任务字符串用作低层监督，否则保持样本不变。对于已标注样本，如果没有适用的配方分支且不存在任务回退，渲染会返回 `None`，从而允许加载器重试另一个样本。
